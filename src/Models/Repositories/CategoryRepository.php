...
class CategoryRepository extends Repository
{
    ...
    {
        $this->instance  = App::make(config('wk-core.class.morph-category.category'));
        $this->morphType = App::make(config('wk-core.class.morph-category.morphType'))::getCodes('relation');
    }
    ...
    public function list(?string $host_type, ?int $host_id, string $code, array $data, $is_enabled = null, $target = null, $target_is_enabled = null, $exceptData = [], $auto_packing = false)
    {
        ...
        $repository = $instance->with(['langs' => function ($query) use ($code) {
                                ...
                                ->when(
                                    config('wk-morph-category.onoff.morph-tag')
        ...
        if ($auto_packing) {
            $factory = new PackagingFactory(config('wk-morph-category.output_format'), config('wk-morph-category.pagination.pageName'), config('wk-morph-category.pagination.perPage'));
            $factory->setFieldsLang(['name', 'description']);

            if (in_array(config('wk-morph-category.output_format'), ['array', 'array_pagination'])) {
                switch (config('wk-morph-category.output_format')) {
                    case "array":
    ...
    public function listOption(?string $host_type, ?int $host_id, string $code, string $code_default, $type = null, $id = null, $degree = 0): array
    {
        ...
        $records = $instance->with(['langs' => function ($query) {
                            ...
                            ->orderBy('order', 'ASC')
                            ->select('id', 'attribute_set', 'serial', 'identifier')
                            ->get();
        $list = [];
